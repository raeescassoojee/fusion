...
class AwsSink:
    ...
    def __init__(self, config: AwsConfig):
        ...
        self.config = config
        self.bucket = os.getenv("SENTINEL_EVIDENCE_BUCKET", config.bucket)
        self.region = os.getenv("AWS_REGION", config.region)
        self.ingestion_url = os.getenv("SENTINEL_INGESTION_URL", config.ingestion_url)
        if not self.bucket:
            raise ValueError(
                "aws.bucket or SENTINEL_EVIDENCE_BUCKET must be configured"
            )
        try:
            import boto3
