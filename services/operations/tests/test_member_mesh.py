...
def test_three_demo_members_and_persistent_cameras(tmp_path: Path, monkeypatch):
    ...
    assert [user["display_name"] for user in payload["users"]] == ["Thandi", "Amy", "Fatima"]
    ...
    assert cameras.json()["cameras"][0]["household"] == "10 Ness Avenue"

    mesh = client.get("/api/member/mesh-state")
    assert mesh.status_code == 200
    camera_rows = mesh.json()["cameras"]
    assert {row["household"] for row in camera_rows} == {
        "10 Ness Avenue", "11 Ness Avenue", "12 Ness Avenue",
    }
    assert {row["display_name"] for row in camera_rows} == {"Thandi", "Amy", "Fatima"}
    assert {
        (row["display_name"], row["household"], row["latitude"], row["longitude"])
        for row in camera_rows
    } == {
        ("Thandi", "10 Ness Avenue", -26.18119, 28.28998),
        ("Amy", "11 Ness Avenue", -26.18151, 28.29062),
        ("Fatima", "12 Ness Avenue", -26.1819, 28.29107),
    }
...
def test_offline_demo_geocoder(tmp_path: Path, monkeypatch):
    ...
    response = client.get("/api/member/geocode", params={"q": "11 Ness Avenue"})
    assert response.status_code == 200
    data = response.json()
    assert data["suburb"] == "Lakefield"
    assert data["latitude"] == -26.18151
    assert data["longitude"] == 28.29062
...
def test_local_detector_repeat_match_and_incident_watch(tmp_path: Path, monkeypatch):
    ...
    assert statuses["10 Ness Avenue"] == "ORIGIN_CONFIRMED"
    assert statuses["11 Ness Avenue"] == "WATCH_ACTIVE"
    assert statuses["12 Ness Avenue"] == "WATCH_ACTIVE"

    # Age the confirmed origin enough for a person to reach the next real Ness
    # Avenue camera. This preserves the production travel-safety gate while also
    # proving that the visible continuity line has two distinct endpoints.
    from sentinel_ops.storage import connect
    with connect() as db:
        db.execute(
            "UPDATE face_sightings SET captured_at=? WHERE sighting_id=?",
            (
                (datetime.now(timezone.utc) - timedelta(seconds=45)).isoformat(),
                first.json()["sighting_id"],
            ),
        )
    ...
    assert second.json()["journey"]["plausible"] is True
    ...
    assert status_by_house["11 Ness Avenue"] == "MATCH_CAPTURED"
    active = mesh.json()["active_incidents"][0]
    accepted_sightings = {
        active["origin_sighting_id"],
        next(
            item["captured_sighting_id"]
            for item in notifications
            if item["household"] == "11 Ness Avenue"
        ),
    }
    trail = next(item for item in mesh.json()["trails"] if item["profile_id"] == active["profile_id"])
    points = [item for item in trail["points"] if item["sighting_id"] in accepted_sightings]
    assert [item["camera_id"] for item in points] == ["CAM-U1-01", "CAM-U2-01"]
    assert len({(item["latitude"], item["longitude"]) for item in points}) == 2
