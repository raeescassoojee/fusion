...
def build_metro_patrol(
    ...
    *,
    ...
    pilot_radius_km: float | None = None,
):
    ...
    # A metro-wide list can span more than 100 km and makes a poor patrol demo.
    # Default to a local pilot cluster around the chosen depot. If that leaves too
    # few stops, add the nearest hotspots until the route has at least three.
    scoped = list(hotspots)
    if pilot_radius_km is not None:
        ranked = sorted(
            hotspots,
            key=lambda item: haversine_km(depot.location, item.location),
        )
        scoped = [
            item for item in ranked
            if haversine_km(depot.location, item.location) <= pilot_radius_km
        ]
    ...
    scoped_ids = {item.hotspot_id for item in scoped}
    baseline = [item for item in baseline if item in scoped_ids]
    requested_stops = max_stops or min(3, len(scoped))
    request = PatrolRequest(
        start=depot.location,
        hotspots=scoped,
        baseline_order=baseline,
        max_stops=min(requested_stops, len(scoped)),
        fuel_l_per_100km=fuel_l_per_100km,
        return_to_start=True,
    )
    comparison = optimise_patrol(request)
    return comparison, scoped, depot, metadata
